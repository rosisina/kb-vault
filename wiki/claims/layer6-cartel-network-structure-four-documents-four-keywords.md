# 군 검찰단 사기수사의 4대 문서·4대 키워드와 카르텔 네트워크 3단계 시간 전개

**Source:** raw/01. book-beyond-cybersecurity/vault-converted-korean/12-3-6-36-제6층위-군.md (§3.6.6.1, lines 791–804)
**Layer:** [[../layers/layer-6|Layer 6]] (primary)
**Aurora node:**
```cypher
MERGE (fr:FalsificationResult {resultId: "FR-L6-B3-002"})
SET fr.layer = 6,
    fr.claimType = "conspiracy_structure",
    fr.claimSubtype = "cartel_network_four_documents_four_keywords",
    fr.claimDesc = "The military prosecutor's fraudulent investigation against Han Ji-hoon is structurally defined by four documents and four keywords. The four documents are: (1) 참고인진술서 (2022-01-25, Record No. 4,776ff), (2) 압수·수색·검증 영장 (2022-07-18, Record No. 4,842ff), (3) 군인·공무원범죄 수사개시 통보 (2022-07-21, Record No. 4,854ff), (4) 피의자 신문조서 (2022-09-02, Record No. 4,874ff), plus (5) 불기소이유 통지 (2022-10-11, Record No. 5,167ff). The four fraud keywords pervading these documents are: 'DIDC 예규 미준수,' '불법적 방화벽 개방,' 'DB 불법접속,' '미고지.' The cartel network operated in three temporal phases: Phase 1 (2016-2018) cover-up foundation, Phase 2 (2018-2022) manipulation system activation via 新KIATIS, Phase 3 (2022-2025) complete-crime completion via prosecutor fraud investigation. The network radiates from 국방부 정보화기획관실 at center, with 국전원, DIDC, 국유단, KIDA, 사이버작전사령부, 국군방첩사령부, 조사본부, and 군 검찰단 as coordinated nodes. Each institution had a specific role: MND (overall command + document/directive manipulation), 국전원 (project manipulation), DIDC (technical concealment), 국유단 (user camouflage + 80 additional requirements), KIDA (academic justification), 사이버사 (security whitewash), 조사본부 + 법무관리관실 (false harassment justification), 국군방첩사 (delayed then fabricated security countermeasure), 군 검찰단 + 최동욱 변호사 (legal finalization). The structure exhibits 'distributed responsibility, centralized control' — the hallmark of organized crime.",
    fr.counterHypothesis = "The multiple organizations acted independently in their respective domains without central coordination from 정보화기획관실; temporal alignment of their actions reflects normal bureaucratic sequencing rather than organized conspiracy.",
    fr.falsificationCondition = "Production of evidence showing that (a) 정보화기획관실 had no communication or coordination role with DIDC/국유단/국군방첩사 regarding the KIATIS investigation, (b) each organization's actions can be explained by independent institutional decision-making without cross-organizational communication, and (c) the 4 keywords in the prosecution documents were derived from independent evidence-gathering rather than a shared narrative framework.",
    fr.verdict = "CORROBORATED",
    fr.strength = "STRONG",
    fr.truthfulness = 9,
    fr.validity = 8,
    fr.sincerity = 10,
    fr.analysisDate = date("2026-04-11"),
    fr.summary = "The prosecution's fraud investigation is structurally mapped through 4 documents (참고인진술서/영장/수사개시통보/피의자신문조서 + 불기소이유통지) and 4 keywords (DIDC예규미준수/불법방화벽/불법DB접속/미고지). The cartel operated in 3 phases (2016-18 cover-up, 2018-22 manipulation, 2022-25 completion) with 정보화기획관실 as central node coordinating 8+ organizations.";
```

## Claim

군 검찰단의 한지훈에 대한 사기수사는 **4대 문서**와 **4대 키워드**로 구조적으로 정의된다.

**4대 문서 (+ 최종 처분):**

| # | 문서 | 일자 | 기록 |
|---|---|---|---|
| 1 | 참고인진술서 | 2022-01-25 | Record No. 4,776ff |
| 2 | 압수·수색·검증 영장 | 2022-07-18 | Record No. 4,842ff |
| 3 | 군인·공무원범죄 수사개시 통보 | 2022-07-21 | Record No. 4,854ff |
| 4 | 피의자 신문조서 | 2022-09-02 | Record No. 4,874ff |
| 5 | 불기소이유 통지 | 2022-10-11 | Record No. 5,167ff |

**4대 사기수사 키워드:** "DIDC 예규 미준수", "불법적 방화벽 개방", "DB 불법접속", "미고지"

카르텔 네트워크는 **국방부 정보화기획관실을 중심**으로 8개 기관이 방사형으로 연결되어 **3단계 시간 전개**로 작동했다:
- **1단계(2016~2018)**: 은폐 기반 구축 — DIDC 해킹 진실 은폐, Active-X 제거사업을 통한 증거 인멸, 훈령 조작
- **2단계(2018~2022)**: 조작 시스템 가동 — 新KIATIS 사업추진체계 조작, 시험평가 왜곡, 한지훈 표적 설정
- **3단계(2022~2025)**: 완전범죄 완성 — 허위 갑질수사 후 군 검찰단 사기수사로 증거 은폐 및 개인 책임 전가

구조의 핵심 특성은 **"분산된 책임, 집중된 통제(distributed responsibility, centralized control)"** — 완전범죄의 전형적 구조이다.

## Key Takeaways

- [진리성] 4대 문서의 시간적 전개(2022-01→07→07→09→10)는 참고인→영장→수사개시→신문→처분의 체계적 사기수사 파이프라인. / The 4 documents' temporal progression (Jan→Jul→Jul→Sep→Oct 2022) forms a systematic fraud-investigation pipeline.
- [진리성] 4대 키워드("DIDC 예규 미준수", "불법 방화벽", "불법 DB접속", "미고지")는 시험평가 환경의 조작된 문서 4건(§3.4 〈그림 4-7〉)에서 파생된 공유 내러티브 프레임워크. / The 4 keywords derive from a shared narrative framework rooted in the 4 fabricated test-evaluation documents.
- [진리성] 3단계 시간 전개(은폐 기반→조작 가동→완전범죄)는 2016 DIDC 해킹에서 2025까지 이어지는 9년간의 조직적 범죄. / The 3-phase temporal progression (cover-up foundation → manipulation activation → complete crime) spans 9 years from the 2016 DIDC hacking to 2025.
- [진실성] 한지훈은 "압수수색 이후 밤낮을 새우며 분석하여 진실을 밝힌" 사람인데 오히려 범죄자가 되었다 — 반부패 정의의 완전한 전도. / Han Ji-hoon, who "worked day and night after the search and seizure to analyze and reveal the truth," was instead made a criminal — complete inversion of anti-corruption justice.
- [타당성] "분산된 책임, 집중된 통제" 구조는 조직범죄학의 표준 분석 틀과 일치. / The "distributed responsibility, centralized control" structure matches standard organized crime analysis frameworks.

## Supporting evidence

- **Record No. 4,776ff / L6** — 참고인진술서 (2022-01-25)
- **Record No. 4,842ff / L6** — 압수·수색·검증 영장 (2022-07-18)
- **Record No. 4,854ff / L6** — 수사개시 통보 (2022-07-21)
- **Record No. 4,874ff / L6** — 피의자 신문조서 (2022-09-02)
- **Record No. 5,167ff / L6** — 불기소이유 통지 (2022-10-11)
- **Book §3.6.6.1** — 전체 카르텔 네트워크 구조 추적도 (〈그림 3-6-3〉)
- **Book §3.4 〈그림 4-7〉** — 조작된 문서 4건과 표적수사 협착관계

## Counter-hypothesis

다수 기관은 각자 관할 범위 내에서 독립적으로 행동했으며 정보화기획관실의 중앙 조율 없이 관료제의 정상적 순차 처리로 시간적 일치가 나타난 것이다. 4대 키워드는 공유 내러티브가 아니라 독립적 증거 수집에서 자연적으로 도출된 공통 발견이다.

**반증 근거 3요소:**
1. 정보화기획관실이 DIDC·국유단·국군방첩사와 KIATIS 수사 관련 소통·조율을 전혀 하지 않았음을 보여주는 증거
2. 각 기관이 교차 조직 소통 없이 독립적 기관 판단으로 동일 결론에 도달했음을 보여주는 의사결정 기록
3. 4대 키워드가 공유 내러티브 프레임워크가 아닌 독립적 증거 수집에서 도출되었음을 보여주는 수사 기록

## Falsification condition

정보화기획관실과 8개 기관 사이에 KIATIS 수사 관련 소통 기록이 전무하고, 각 기관이 독립적 증거에 기반한 자율 판단으로 동일한 4대 키워드에 도달했음을 보여주는 내부 기록이 제시될 경우, 이 주장은 약화된다.

## Verdict

**CORROBORATED.** Strong. 진리성 9 / 타당성 8 / 진실성 10. 4대 문서·4대 키워드의 구조적 일관성과 3단계 시간 전개는 우연의 일치로 설명 불가능하다. 다만, 저자 자신이 인정하듯 증거기록은 한지훈 1인에 의해 수집된 것으로 완전성에 한계가 있으며, 본격적 검증은 국가 수사기관에 의해 이루어져야 한다.

## Open Questions

- 新KIATIS 서버 구축사업에서 DIDC-국방부 직접 조율이 국전원을 우회한 구체적 경위 (footnote 583)
- 국유단 상사 장우진 (사업실무자-1)의 서버사업 참여 수준이 낮았다는 판단의 구체적 근거
- 〈그림 3-6-3〉의 시각적 내용 미확인 (텍스트 기술만 확보)

## Spot-check (raw/01 book)

- `vault-converted-korean/12-3-6-36-제6층위-군.md` §3.6.6.1 (lines 791–804): 4대 문서(기록 제4,776/4,842/4,854/4,874/5,167), 4대 키워드, 3단계 시간 전개, 8개 기관 역할 분담, "분산된 책임 집중된 통제" — 모두 일치.

## Related

- [[layer3-vpn-otp-card-used-2021-not-2019|3 shared records — VPN OTP 카드 L3 사실관계]] (RELATED)
- [[prosecution-distorts-operational-vs-test-environment|4 shared records — 환경 왜곡 관련]] (RELATED)
- [[han-ji-hoon-witness-statement-2022-01-25|4 shared records — 참고인 진술 원본]] (RELATED)
- [[../layers/layer-6|Layer 6]] (PART_OF_LAYER)
- [[defense-information-cartel-named-by-rebuttal]] — 카르텔 정의 (OPPOSES)
- [[han-ji-hoon-investigation-notification-2022-07-21]] — 수사개시 통보 (RELATED)
- [[prosecution-misapplies-2129-article-58-4-to-kiatis]] — 불기소이유서 오적용 (RELATED)
- [[layer6-prosecutor-violated-anti-corruption-five-principles]] — 반부패 5대 원칙 위반 (RELATED)
- [[../entities/people/han-ji-hoon|한지훈]] (ABOUT)
