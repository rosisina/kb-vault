...
**Layer:** [[../layers/layer-5|Layer 5]] (primary — 인권침해·가스라이팅), [[../layers/layer-7|Layer 7]] (secondary)
**Aurora node:**
```cypher
MERGE (fr:FalsificationResult {resultId: "FR-L5-PSYCHIATRIC-GASLIGHTING"})
SET fr.layer = 5,
    fr.secondaryLayers = [7],
    fr.claimType = "human_rights_violation",
    fr.claimSubtype = "gaslighting_psychiatric_abuse",
    fr.claimDesc = "국전원 관리과 천하경(주무관)이 반복적으로 한지훈에게 '휴직을 해라, 정신병원을 가라 왜 안가냐'고 권유하고, 최동욱 변호사도 '지금 판단하는 것은 스트레스 때문에 정상적이지 않다'고 한지훈의 정신 상태를 문제시. 한지훈 반박: '정상적이지 않은 것은 제가 아니예요... 32년 된 사람이 지금 한 거를(독방 생활) 깜짝 놀랐지 않습니까'. 6개월 독방 격리 + 사이버안보훈령 위반 기소 + 아들 조현병이라는 극한 상황에서 피해자를 '비정상'으로 프레이밍하는 것은 피해자의 증언 신뢰도를 사전 훼손하려는 조직적 가스라이팅",
    fr.counterHypothesis = "천하경과 최동욱은 한지훈의 정신건강을 진심으로 우려하여 권유한 것이다",
    fr.falsificationCondition = "한지훈이 정신건강 검진을 받고 '비정상' 판정을 받은 공식 의료 기록, 또는 천하경의 휴직 권유가 한지훈의 복리를 위한 공식 인사 조치였음을 보여주는 기록",
...
> **(한지훈→최동욱, 녹취 타 line 8625~8635):**
> "천하경(주무관) 자꾸 나한테 **'휴직을 해라' '그 쪽을 (정신병원) 가라 왜 안가냐'**..."

### 최동욱의 "정상적이지 않다" 프레이밍

> **(최동욱, Record No. 11,252, 기존 atom에서 인용):**
> "**지금 판단하고 이런 것은 이런 스트레스 때문에 정상적이지 않다**"

### 한지훈의 반박

> **(한지훈):**
> "**정상적이지 않은 것은 제가 아니예요**... 32년 된 사람이 지금 한 거를(독방 생활) 깜짝 놀랐지 않습니까. 장로님도 그런데 **조치를 안 하시잖아요**"

### 가스라이팅의 이중 구조

| 행위자 | 발언 | 역할 |
|---|---|---|
| **천하경** (국전원 관리과) | "휴직해라, 정신병원 가라" | **조직 내부**에서 피해자를 '비정상'으로 낙인 |
| **최동욱** (변호사) | "정상적이지 않다" | **법적 대리인**이 피해자의 판단력을 부정 |
