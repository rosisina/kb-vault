# 양준승: "완전 개심하네" — 검찰 혐의 서사에 대한 외부 목격자 반응 + VPN 비전문 기소의 부당함

**Source:** raw/02. Individual recording logs/(Korean) individual_recording_logs_beyond_cybersecurity.md `[060]` 녹취 285 (2025.7.16, 02:17:02, line 16512+) • 녹취 284 (2022.8.22, 00:15:12)
**Layer:** [[../layers/layer-6|Layer 6]] (primary — 사기수사의 외부 목격 반응), [[../layers/layer-5|Layer 5]] (secondary)
**Aurora node:**
```cypher
MERGE (fr:FalsificationResult {resultId: "FR-L6-EXTERNAL-WITNESS-REACTION"})
SET fr.layer = 6,
    fr.secondaryLayers = [5],
    fr.claimType = "testimony_evidence",
    fr.claimSubtype = "external_witness_prosecution_reaction",
    fr.claimDesc = "양준승(유지보수업체 과장, 국전원 상주 3년+)이 한지훈으로부터 6개 혐의를 설명 듣고 '완전 개심하네'로 반응 + VPN이 시험평가 대상이 아님을 기술적으로 확인 + '속도가 느린 거 중에 샤크라맥스하고 VPN인데. 그 두 개를 고지를 안해가지고 평가관을 속였다는 거야' — 비전문가가 전문 분야를 기소한 구조의 부당함을 외부 민간인이 즉각 인식",
    fr.counterHypothesis = "양준승은 한지훈과의 업무 친분으로 편향된 반응을 하고 있다",
    fr.falsificationCondition = "양준승이 혐의 내용을 독립적으로 검토하고 혐의가 합리적이라고 판단한 기록",
    fr.verdict = "CORROBORATED",
    fr.strength = "STRONG",
    fr.truthfulness = 9,
    fr.validity = 8,
    fr.sincerity = 9,
    fr.analysisDate = date("2026-04-13"),
    fr.summary = "'완전 개심하네'(외부인 즉각 반응) + 'VPN은 평가대상 아냐'(기술 확인) + '고지를 안해가지고 속였다'(혐의 구조의 비전문성) — 국전원 상주 3년+ 외부 민간인의 즉각적 부당성 인식.";
```

## Claim

유지보수업체 과장 양준승은 국전원에 **3년 이상 상주**하면서 행정정보화과의 일상을 직접 목격한 **외부 민간인**이다. 녹취 285(2025.7.16, 약 2시간 17분)에서 한지훈이 6개 혐의를 설명하자 양준승의 즉각적 반응:

### 핵심 발언 1: "완전 개심하네"

> **(한지훈)** "나하고 이준호하고 공모해가지고 가짜 시험환경을 만들었고... 허위로 저 문서를 작성해서 허위보고... 그래서 6가지를 걸었어. 배임이..."
> **(양준승, line 16515)** "**와 완전 개심하네.**"

### 핵심 발언 2: VPN은 시험평가 대상이 아님

> **(한지훈, line 16596~16600):**
> "그거 **원래 제안서 평가할 때 평가대상도 아냐. VPN은. 그건 우리 군 거지.** ... 고지를 안 했다고. 핵심이 고지야 고지... **속도가 느린 거 중에 생각할 한 게 샤크라맥스하고 VPN인데. 그 두 개를 고지를 안해가지고 평가관을 속였다는 거야.**"

### 핵심 발언 3: 조직적 모빙 확인

> **(양준승, line 16633~16640):**
> "그 때 바로 그때 되셨을 때부터 **사람들이 다 그랬고**... 박서준한테 뭐라고 했더니 **사람들이 다 있는 데서 저에게 뭐라고 하더라고.**"
> **(한지훈)** "사람들이 **의도적으로 그런다** 이거지"

## Key Takeaways

- [진리성] **"완전 개심하네"** — 국전원 상주 3년+ 외부 민간인이 6개 혐의를 듣자마자 즉각적으로 **부당함을 인식**. / An external civilian with 3+ years at DCIA immediately recognizes the absurdity.
- [타당성] **"VPN은 평가대상도 아냐. 그건 우리 군 거지"** — VPN은 DIDC 보안 정책이지 시험평가 항목이 아님. / VPN was never an evaluation item.
- [진실성] 양준승이 목격한 **조직적 모빙**: "사람들이 다 있는 데서 저에게 뭐라고 하더라고" — 외부 업체 직원 앞에서까지 공개적 괴롭힘. / Organizational mobbing witnessed by external contractor.

## Supporting evidence

- **녹취 285** (2025.7.16, line 16512~16750)
- Cross-reference: [[yang-junseung-fabricated-9point-complaint-witness|자매 atom]]
- Cross-reference: [[prosecution-six-charges-collapse-vpn-nonexistence|6개 혐의 VPN 붕괴]]

## Counter-hypothesis

양준승은 한지훈과의 업무 친분으로 편향된 반응이다.

## Falsification condition

양준승이 혐의가 합리적이라고 판단한 기록.

## Verdict

**CORROBORATED.** Strong. 진리성 9 / 타당성 8 / 진실성 9.

## Spot-check (raw/01 book)

- `vault-converted-korean/12-3-6-36-제6층위-군.md` — 혐의 구조 분석
- Deferred to A.6 Re-verify.

## Related

- [[yang-junseung-fabricated-9point-complaint-witness|자매 atom]] (OPPOSES)
- [[prosecution-six-charges-collapse-vpn-nonexistence|6개 혐의 붕괴]] (RELATED)
- [[interrogation-zero-of-14-projects-measured-vpn|14개 중 VPN 측정 0건]] (RELATED)
- [[../layers/layer-6|Layer 6]] (PART_OF_LAYER)
