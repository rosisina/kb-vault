...
In the '2019 DB Access Control (ChakraMax) Operation Status Check Plan' produced and sent by DIDC Center 1 on 2019-02-01 (Record No. 1,874), 舊KIATIS is listed in the defense network list as a 'VPN user.' However, as proven in Layer 1, 舊KIATIS at that time was still operated without VPN within an internet homepage server. Therefore this entry is false and fabricated (Layer 4 Theorem 1).

This inspection plan also lists over 100 internet and defense network target servers (Record No. 1,875) with the phrase 'limited to DB server access administrators among VPN users,' and its substantive content is 'blanket blocking' of systems not enrolled in the DB access control system (ChakraMax).
